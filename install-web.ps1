# Kimi Code Desktop (WEB EDITION) - one-command installer (Windows)
#
...
# Kimi CLI's own `kimi web` chat interface. Installs SILENTLY via the
...
# The NSIS Setup (electron-builder) installs into Programs\kimi-cli-desktop-web
# - derived from the package name, NOT the product name - so the CLI edition
# (Programs\kimi-cli-desktop) and this one can sit side by side.
$SetupDir   = if ($env:KCD_INSTALL_DIR) { $env:KCD_INSTALL_DIR.TrimEnd("\") }
              else { Join-Path $env:LOCALAPPDATA "Programs\kimi-cli-desktop-web" }
...
function Main {
    ...
    # WEB EDITION assets only ("Kimi-Code-Desktop-Web-...") - a CLI-edition
    # release must never be picked up by mistake. Prefer the NSIS Setup: it
    # installs properly (Start Menu entry + uninstaller). The portable exe is
    # only a fallback for odd cases.
    $asset = $release.assets |
        Where-Object { $_.name -match "Web-Setup-.*x64\.exe$" } | Select-Object -First 1
    if (-not $asset) {
        $asset = $release.assets |
            Where-Object { $_.name -match "Web-.*portable\.exe$" -and $_.name -match "x64" } |
            Select-Object -First 1
    }
    if (-not $asset) {
        $asset = $release.assets | Where-Object { $_.name -match "Web-.*portable\.exe$" } | Select-Object -First 1
    }
    if (-not $asset) {
        throw "No WEB EDITION Windows build in the latest release ($($release.tag_name)). Check $ReleasePage - the Web edition publishes 'Kimi-Code-Desktop-Web-*' assets."
    }
