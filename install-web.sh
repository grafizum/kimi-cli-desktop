#!/usr/bin/env bash
#
# Kimi Code Desktop (WEB EDITION) — one-command installer (Linux / macOS)
#
#   curl -fsSL https://raw.githubusercontent.com/grafizum/kimi-cli-desktop/kimi-web-desktop/install-web.sh | bash
#
# Installs the WEB EDITION (branch kimi-web-desktop): the app that embeds the
# Kimi CLI's own `kimi web` chat interface. Downloads the latest Web-edition
# release from GitHub and installs it:
#   • Linux  → ~/.local/bin/Kimi-Code-Desktop-Web-<version>-<arch>.AppImage
#              (plus a desktop entry so it shows up in your app launcher)
#   • macOS  → ~/Downloads/Kimi-Code-Desktop-Web-<version>-<arch>.dmg (opened for you)
#
...
RAW="https://raw.githubusercontent.com/${REPO}/kimi-web-desktop"
...
case "$OS" in
  ...
  # named "x86_64", macOS/Windows ones are named "x64". WEB EDITION assets
  # only ("...-Web-..."): a CLI-edition release must never be picked up.
  linux) ASSET="$(printf '%s\n' "$ASSET_URLS" | grep -F "Web-" | grep -F ".AppImage" | grep -F "$ARCH" | head -n 1 || true)" ;;
  macos)
    MAC_ARCH="x64"; [ "$ARCH" = "arm64" ] && MAC_ARCH="arm64"
    ASSET="$(printf '%s\n' "$ASSET_URLS" | grep -F "Web-" | grep -F ".dmg" | grep -F "$MAC_ARCH" | head -n 1 || true)" ;;
esac

[ -n "$ASSET" ] || die "No WEB EDITION asset for ${OS}/${ARCH} yet. The Web edition publishes 'Kimi-Code-Desktop-Web-*' assets — check the releases page, or install from source (see the README)."
...
APPIMAGE="$BIN_DIR/Kimi-Code-Desktop-Web.AppImage"
mkdir -p "$BIN_DIR"
...
if ! "$APPIMAGE" --appimage-version >/dev/null 2>&1; then
  ...
  EXTRACT_DIR="$HOME/.local/share/kimi-cli-desktop-web"
  mkdir -p "$EXTRACT_DIR"
  ( cd "$EXTRACT_DIR" && "$APPIMAGE" --appimage-extract >/dev/null 2>&1 )
  cat > "$BIN_DIR/Kimi-Code-Desktop-Web" <<EOF
#!/usr/bin/env bash
exec "$EXTRACT_DIR/squashfs-root/AppRun" "\$@"
EOF
  chmod +x "$BIN_DIR/Kimi-Code-Desktop-Web"
  APPIMAGE="$BIN_DIR/Kimi-Code-Desktop-Web"
fi

# Desktop entry so it appears in the app launcher.
DESKTOP_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/applications"
mkdir -p "$DESKTOP_DIR"
download "$RAW/assets/icon.png" "$DESKTOP_DIR/kimi-cli-desktop-web.png" 2>/dev/null || true
cat > "$DESKTOP_DIR/kimi-cli-desktop-web.desktop" <<EOF
[Desktop Entry]
Name=Kimi Code Desktop (Web)
Comment=The Kimi Code CLI's chat interface, as a desktop app
Exec=$APPIMAGE %F
Icon=$DESKTOP_DIR/kimi-cli-desktop-web.png
Terminal=false
Type=Application
Categories=Development;Utility;
StartupWMClass=Kimi Code Desktop
EOF
chmod +x "$DESKTOP_DIR/kimi-cli-desktop-web.desktop"
